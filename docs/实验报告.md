...
2、对每个物体判断射线是否穿过该物体，这里我们把物体看成一个长方体，然后对每个轴判断射线进入时间和出去时间，如果一个射线穿入该物体，那么他的最晚进入时间一定小于等于最早出去时间，由此可以判断物体是否在射线范围内，具体见代码 `gameobject.h/RayIntersect(constglm::vec3&rayOrigin, constglm::vec3&rayDir, floatmaxDistance=1000.0f)`
